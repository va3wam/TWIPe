 ...
 *                    -add another debug print command: AMDP_PRINTF(x,y)
  *                   -left in the mpu.Calibrate... calls, although they're absent in Rowberg's latest demo code
  *                   -require the MPU6050 library with the fix on line 2764, by renaming it to MPU6050-fix2764.*. Had to change
  *                     MPU6050-6Axis_MotionApps_V6_12 as well, because it references MPU6050.h
  *                   -reformat #includes to clarify which libraries we created ourselves, and make lines < 140 characters
  *                   -remove all support for DMP interrupts, which latest Rowberg code doesn't need or use
  *                   -select DMP's YPR[2] as best tilt angle to use, in variable tilt, and trim code for other 2 methods
 ...
 * 0.0.2   2020-04-27 Special trimmed down build that just does IMU output to the serial port
...
#define tmrIMU 50        // Milliseconds to wait between reading data to IMU over I2C and calculating angles
...
/**
>  * @brief Process a stored Wifi Event 
>  ================================================================================================ */
void processWifiEvent()                    // called fron loop() to handle event ID stored in WifiLastEvent
{
  ...
  AMDP_PRINT("<WiFiEvent> processing event:");  
  AMDP_PRINTLN(event);
  switch(event) 
  {
    ...
    case SYSTEM_EVENT_STA_DISCONNECTED:
    {
      ...
      wifiDropCnt ++; // Increment the number of network drops that have occurred
      break;      
    } //case
    case SYSTEM_EVENT_STA_GOT_IP:
    {
      ...
      Serial.print("<WiFiEvent> Network connection attempt #");
...
void updateMetaData()
{
  ...
  AMDP_PRINTF("  Millis: %6lu",millis());
  AMDP_PRINTF("  Tilt: %6.2f\n", tilt);
  goMETADATA = millis() + tmrMETADATA; // Reset SERIAL update target time
} // updateMetaData()

/**
 * @brief Update OLED dipsplay 
 * @param: none. reads global variable tilt, which is in degrees 
 * @note We are working with Yaw/Pitch/Roll data (and only using pitch). Other options include euler, quaternion, raw acceleration, 
 * raw gyro, linear acceleration and gravity. See MPU6050_6Axis_MotionApps_V6_12.h for more details.   
================================================================================================================ */
void updateOLED()
{
  rightOLED.clear();        
  rightOLED.drawString(64, 20,  "tilt: " + String(tilt));
  rightOLED.display();  
...
void setupOLED()
{
  ...
  // original, before Doug started experimenting
  // rightOLED.setFont(ArialMT_Plain_24);
  // rightOLED.drawString(64, 0, "My Demo"); //64,22
...
// Rowbergs latest example sets up DMP interrupt, but doesn't use it. We'll leave interrupt support out.
  ...
  if (devStatus == 0) 
  {
    // your own gyro offsets here, from the cfgbyMAC routine
    ...
    // next 2 calls not in the Rowberg example, but leaving them in for now
  ...
  else // If initialization failed
  {
    ...
    // TODO improve handling of case where IMU has a startup problem.
  } //else
  Serial.println("<setupIMU> DMP Initialization succeeded");  // explicit message for console log
} //setupIMU()
...
// after IMU rotated 180 degrees around pin holes
    XGyroOffset = 55;
    YGyroOffset = -10;
    ZGyroOffset = -55;
    XAccelOffset = -1995;
...
/**
 * @brief Flash AMBER LED on front panel button
================================================================================================================ */
void updateLED()
{
...
void readIMU()
{
  if (mpu.dmpGetCurrentFIFOPacket(fifoBuffer)) // get the latest DMP FIFO buffer. 
  {  
    ...
    if (tilt < -120.) tilt = 90.;      // avoid abrupt change from +90 to -270, when he's past a face plant
...
void loop() 
{
  ...
  if((millis() >= goIMU) ) readIMU();          // Update the OLED with data, ignoring interrupt stuff
  if(millis() >= goOLED) updateOLED();         // Update the OLED with data in tilt variable
  if(millis() >= goLED) updateLED();           // Update the blinking LED 
  if(millis() >= goMETADATA) updateMetaData(); // Send data to serial terminal & maybe MQTT
} //loop()